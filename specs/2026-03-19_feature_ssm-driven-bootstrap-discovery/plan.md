...
**What:**
- Add SSM parameter `/conga/config/conga-image` storing `var.conga_image`
...
**Testable independently:** Yes — `terraform apply`, verify params with `aws ssm get-parameter`. `conga admin list-agents` still works (extra JSON fields are ignored by the CLI's unmarshal).
...
```hcl
[for k, v in local.user_agents : "...conga/${v.member_id}/*"]
```
with wildcards:
```hcl
"...conga/U*"   # user secrets (member IDs always start with U)
"...conga/teams/*"  # team secrets
```
...
**Architect note:** This is slightly broader than per-user enumeration but still scoped to `conga/` prefix. The instance already has `ListSecrets` on `*`. The incremental risk is minimal — single-tenant infrastructure where any process could already enumerate secrets.
...
b. **Read `conga_image` from SSM (section 4):**
   Replace `CONGA_IMAGE="${conga_image}"` with:
   ```bash
...
d. **Replace section 6 entirely:**
   ...
   - Query `/conga/users/` and `/conga/teams/` via `aws ssm get-parameters-by-path`
   - Filter out `/conga/users/by-iam/*` params (these are IAM mappings, not agent configs)
...
**What (router.tf):**
   Remove `agents`, `agent_container_id`, `conga_image`, and `routing_json` from the `templatefile()` call. Remaining vars: `aws_region`, `project_name`, `config_check_interval_minutes`, `state_bucket`.

**QA edge cases:**
- `/conga/users/by-iam/*` params must be filtered out — they're IAM mappings, not agent configs
...
- **`outputs.tf`** — SSM port-forward commands use `var.agents`. CLI agents use `conga connect` which reads SSM directly.
- **`secrets.tf`** — cleanup provisioner stays Terraform-driven. CLI agents cleaned up via `conga admin remove-user --delete-secrets`.
...
| Bootstrap fails if SSM param for conga-image doesn't exist (fresh deploy before step 1 applied) | Steps 1 and 3 must be applied in order. Step 1 creates the param, step 3 reads it. |
...
| Terraform plan loses visibility into "what agents will be provisioned" | `terraform plan` shows SSM param changes. `conga admin list-agents` shows the full picture. Acceptable tradeoff. |
...
- CLI setup scripts read `conga_image` from SSM instead of template var (eliminates last config divergence)
...
- `conga admin sync` command that reads SSM and reconciles with running containers (diagnose drift without rebooting)
