...
- Update `terraform/terraform.tfvars.example` — replace real Slack IDs with `UXXXXXXXXXX`/`CXXXXXXXXXX` placeholders, add `conga_image` variable

### 1b. New variable: `conga_image`
...
- `terraform/router.tf`: Pass `conga_image` and `state_bucket` to the `user-data.sh.tftpl` templatefile call
...
- `terraform/user-data.sh.tftpl`:
  - Replace hardcoded ECR repo (`123456789012.dkr.ecr...`) with `${conga_image}` template variable
...
- `cli/scripts/add-user.sh.tmpl`: Replace `ghcr.io/openclaw/openclaw:latest` with `{{.Conga LineImage}}`
- `cli/scripts/refresh-user.sh.tmpl`: Replace `ghcr.io/openclaw/openclaw:latest` with `{{.Conga LineImage}}`
- `cli/cmd/admin.go`: Add `Conga LineImage` to template execution struct, sourced from `cfg.Conga LineImage`
- `cli/cmd/refresh.go`: Add `Conga LineImage` to template execution struct
...
- `cli/README.md`: Same treatment. Add section about `~/.conga/config.toml` setup with example config
...
| `terraform/variables.tf` | Empty users default, add `conga_image` var |
...
| `cli/scripts/add-user.sh.tmpl` | Use `{{.Conga LineImage}}` |
| `cli/scripts/refresh-user.sh.tmpl` | Use `{{.Conga LineImage}}` |
...
| `conga_image` empty at deploy time | Terraform validation block — require non-empty |
