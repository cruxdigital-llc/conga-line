...
Make the repository publishable as an early-preview open-source project. Anyone should be able to clone it, configure it for their own AWS account and Slack workspace, and deploy Conga Line. Speed over completeness — rough edges are acceptable, but the project must be company-agnostic and fully configurable.
...
4. **Company-agnostic CLI**: No hardcoded org-specific defaults (SSO URL, account ID). Users configure via `~/.conga/config.toml` or env vars
5. **Configurable Docker image**: The Conga Line image name is a variable, not hardcoded — users can point to their own ECR, GHCR, or Docker Hub image (upstream `ghcr.io/openclaw/openclaw:latest` requires PR #49514 bugfix to work with Slack)
...
- CLI config struct cleanly supports new `Conga LineImage` field
...
- Edge case: what happens if `conga_image` is empty? If `users = {}`? If CLI config.toml doesn't exist?
