...
Wait — `%N` is the systemd unit name (e.g. `openclaw-aaron`), but we need the agent name (e.g. `aaron`). Use a variable substitution:
...
```
ExecStartPre=/bin/bash -c 'aws s3 sync s3://openclaw-terraform-state-123456/openclaw/behavior/ /opt/openclaw/behavior/ --region us-east-1 2>/dev/null || true; /opt/openclaw/bin/deploy-behavior.sh aaron $(cat /opt/openclaw/config/aaron.type 2>/dev/null || echo user)'
```
