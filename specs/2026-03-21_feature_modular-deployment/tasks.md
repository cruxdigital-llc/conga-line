...
- [x] **1.1** Create `cli/internal/provider/provider.go` — Provider interface + all shared types
- [x] **1.2** Create `cli/internal/provider/registry.go` — Provider registry (Register, Get, Names)
- [x] **1.3** Create `cli/internal/provider/config.go` — Provider config load/save
- [x] **1.4** Create `cli/internal/common/config.go` — `GenerateOpenClawConfig()` and `GenerateEnvFile()`
- [x] **1.5** Create `cli/internal/common/routing.go` — `GenerateRoutingJSON()`
- [x] **1.6** Create `cli/internal/common/behavior.go` — `ComposeBehaviorFiles()`
- [x] **1.7** Create `cli/internal/common/ports.go` — `NextAvailablePort()`
- [x] **1.8** Create `cli/internal/common/secrets.go` — `SecretNameToEnvVar()`
- [x] **1.9** Create `cli/internal/common/validate.go` — Slack ID validators

## Phase 2: AWS Provider ✅

- [x] **2.1** Create `cli/internal/provider/awsprovider/provider.go` — AWSProvider wrapping existing code
...
- [x] **3.1** Create `cli/internal/provider/localprovider/provider.go` — LocalProvider struct + all methods
- [x] **3.2** Create `cli/internal/provider/localprovider/docker.go` — Docker CLI wrapper
- [x] **3.3** Create `cli/internal/provider/localprovider/secrets.go` — File-based secret storage (mode 0400)
...
- [x] **5.1** Create `cli/internal/provider/localprovider/integrity.go` — Config hash checking + logging
