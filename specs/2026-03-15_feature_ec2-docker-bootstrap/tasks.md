...
3. **uid 1000 collision**: AL2023's `ec2-user` is uid 1000. Created `openclaw` user without fixed uid.
4. **`loginctl enable-linger` failed**: `systemd-logind` not started by default. Fixed by starting it first.
5. **EROFS / read-only filesystem**: OpenClaw hot-reload writes `.tmp` files next to `openclaw.json`. Cannot mount config as `:ro`. Fixed by mounting entire `.openclaw` dir writable with config owned by container user.
...
- Slack: socket mode connected, channel C0ALL272SV8 resolved
