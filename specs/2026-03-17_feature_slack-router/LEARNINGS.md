...
## What We Built
A Node.js Socket Mode proxy that receives all Slack events via a single outbound WSS connection, routes by channel/user ID, and forwards to per-user OpenClaw containers via internal HTTP. Zero ingress preserved.
...
## What Didn't Work
- **OpenClaw HTTP webhook mode returns 404**: Despite correct config (`mode: "http"`, `botToken`, `signingSecret`, `webhookPath` all present) and the log saying `[slack] http mode listening at /slack/events`, POST requests to the endpoint return 404.

## Root Cause Analysis
Traced through the OpenClaw source at github.com/openclaw/openclaw:
...
**This is an OpenClaw bug.** The HTTP webhook handler registers in a different module instance than the one the gateway HTTP server reads from. It affects the compiled Docker image (`ghcr.io/openclaw/openclaw:latest`, version 2026.3.13).
...
### OpenClaw HTTP Mode Requirements
...
## Recommendation
1. **File an OpenClaw issue** about the HTTP webhook mode module identity split bug
2. **Preserve the router code** — it works correctly; the bug is in OpenClaw's HTTP mode, not our router
3. **Revisit when OpenClaw fixes HTTP mode** or when we can build from source with the fix
