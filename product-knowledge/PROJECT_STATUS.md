...
- **Reboot-fragile migrated agents (HIGH, tracked in #33)**: the 3 agents on static `10.99.x` (aaron, nathan, congaline-team) can crash-loop on a host reboot / docker daemon restart because the egress proxy isn't IP-pinned and may grab the agent's `.2`. Do NOT migrate the remaining 3 agents (nextgen-delivery, nvidia-team, zach — safe on auto-subnets) until #33 (proxy pin + migration robustness) lands. Avoid a host reboot until then.
...
- **Docker ghost endpoints**: per-agent bridge networks can accumulate dangling `conga-router` endpoints (pre-host-net-router era) + stale `172.x` DOCKER-USER rules across refreshes; a persisted ghost may need a `docker` daemon restart to clear. #33 R2 addresses the migration's handling; a one-time `172.x` DOCKER-USER sweep is safe once all agents are on `10.99.x`.
