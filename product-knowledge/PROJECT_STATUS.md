...
- **Slack-driven subagent spawn blocked — ROOT-CAUSED (2026-06-14): intentional OpenClaw security fix; fix is on our side**: `sessions_spawn` (Qwen subagent) from a Slack/channel session fails with "missing scope: operator.write"; works from operator surfaces (CLI/Control UI/`conga connect`). Worked on v2026.5.26 (real subagent ran from Slack 2026-05-27), broke on v2026.6.5. **Cause:** OpenClaw deliberately removed the shared-token "backend self-pairing" bypass our in-process spawn relied on — a fix for a critical (CVSS 9.3) local priv-esc vuln (openclaw/openclaw#72418, closed by #86192 merged 2026-05-27 + #90188 merged 2026-06-04, both in 6.5). **NOT a regression to file.** Conga gap: we provision with `gateway.auth.mode:"token"` and never run OpenClaw's device-pairing flow, so the runtime had no device identity and fell back to the now-closed bypass. The fix **preserved** a device-token (and `approvalRuntimeToken`, `auth.mode:"none"`) path. **Fix = a Conga feature: device-token provisioning** → `specs/2026-06-14_feature_agent-device-token-provisioning/` (planning). Full analysis: `specs/2026-06-14_feature_agent-device-token-provisioning/root-cause-analysis.md`. (Aaron's subagent endpoint is a LAN IP reachable from the AWS host only via the VPC VPN route 10.0.0.17 — working from the operator surface.)
...
## Recent Changes
- 2026-06-14: **Agent Device-Token Provisioning — SPECIFIED (spike-gated)** (`/glados:plan-feature` + `spec-feature`). Restores Slack/channel-originated subagent delegation (`sessions_spawn` to a cheaper model e.g. Qwen) that broke on the v2026.6.5 upgrade. **Root-caused (not a regression):** OpenClaw intentionally removed the shared-token "backend self-pairing" bypass our in-process spawn relied on — a fix for critical (CVSS 9.3) priv-esc vuln openclaw/openclaw#72418 (closed by #86192 + #90188, both in 6.5). Phase 0 discovery on the deployed 2026.6.5 found: headless device-token CLI (`openclaw devices rotate`); a paired Control-UI operator device already exists (why operator-surface spawn works); `approvalRuntimeToken` is the preserved device-less path; **obstacle** — `shouldOmitDeviceIdentityForGatewayCall` omits the runtime's device identity when shared auth is present, so the native path is likely `approvalRuntimeToken`, not device-pairing. **Spec mandates a mechanism spike (Task 0) on a live agent before any build** (Candidate A approvalRuntimeToken / B device-token+suppress-shared-auth / C upstream-ask); security review against the #72418 boundary is a blocking gate pre-rollout. Persona review all-APPROVE; standards gate PASS (security conditionally gated on spike). Analysis: `specs/2026-06-14_feature_agent-device-token-provisioning/root-cause-analysis.md`. See `specs/2026-06-14_feature_agent-device-token-provisioning/`.
