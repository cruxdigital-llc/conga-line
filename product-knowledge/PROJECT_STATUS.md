...
- [x] Schema defined (`RoutingPolicy` in `cli/pkg/policy/policy.go`)
...
### 23. Egress Proxy Image Cleanup — Specified, Ready for Implementation
*Lead: Architect*
*See `specs/2026-04-02_feature_egress-proxy-image-cleanup/` for full trace*
- [x] Requirements defined
- [x] Plan defined
- [x] Spec defined
- [x] Persona review passed (PM + Architect + QA)
- [x] Standards gate passed (0 violations, 0 warnings)
- [ ] Implementation (single phase: update constant, remove build machinery, delete Dockerfile)
- [ ] Verification
...
- 2026-03-25: Portable Policy Schema — `conga-policy.yaml` schema for declaring security and routing policy as a portable artifact. New `cli/pkg/policy/` package with YAML parsing (`gopkg.in/yaml.v3`), validation (enum checks, domain format, unknown field rejection), per-agent override merging, and per-provider enforcement reporting. `conga policy validate` CLI command with `--file`, `--agent`, `--output json` support. 5 new files, 19 unit tests. See `specs/2026-03-25_feature_policy-schema/`.
