...
### Remote-MCP OAuth Credential Lifecycle — ✅ COMPLETE (shipped + released + live-verified 2026-07-23)
Both phases done and released (`terraform-provider-conga` v0.1.10 / congaline v0.0.32). Full detail in
`specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/` and the 2026-07-23 Recent Changes entry below.
- **Still-open follow-up — retire `team-b`'s GitHub PAT → OAuth**: its GitHub MCP server uses a static
  `github_pat_…` embedded in `custom.json` (no `auth: oauth`) — contra security.md §5. **Rotate the
  exposed PAT ASAP** (independent of this feature); then switch to OAuth (the new lifecycle machinery
  will manage it). Steps in `spec.md` appendix.
...
- ~~**Remote-MCP OAuth creds are unmanaged per-container state**~~ **RESOLVED 2026-07-23 (v0.1.10)**: `conga doctor` detects OAuth-needed servers, `conga mcp login` re-auths (one command), and the credential blob is now captured to the per-agent secrets store + cold-only restored into the data dir before container start on all three providers. Residual (by design): refresh-token expiry/revocation (Mode B) still needs a browser re-auth — `doctor` surfaces it, `mcp login` fixes it. See `specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/`.
...
## Recent Changes
- 2026-07-23: Remote-MCP OAuth Credential Lifecycle **COMPLETE — shipped + released + live-verified** (congaline **v0.0.32** / `terraform-provider-conga` **v0.1.10**; terraform pin 0.1.9→0.1.10, PR #79). Origin: a live `team-a` "no Linear access" incident where the per-container OAuth credential (`/home/node/.openclaw/mcp-oauth/<server>-<hash>.json`, holds the refresh token) had gone missing — invisible to the declarative fleet, wiped on lifecycle events, silent `bundle-mcp … requires OAuth`. **Phase 1 (PR #74)**: `conga doctor` (fleet OAuth log-scan) + `conga mcp login` (two-leg re-auth), both CLI+JSON+MCP, shared pure logic in `internal/mcpoauth`; plus a new **confidentiality MUST standard** (`standards/confidentiality.md`) after real agent names leaked into PR text/commits (remediated + scrubbed, PR #75, follow-up issue #76). **Phase 2 (PR #77 local + #78 remote/AWS)**: `Runtime.OAuthStateDir()` + `mcp-oauth/` secret prefix (excluded from env-file gen so a token blob never leaks to env); `common.CaptureMCPOAuth` (blob→per-agent secret store, JSON-guarded) wired into `conga mcp login`; `common.RestoreMCPOAuth` **cold-only** (never overwrites the runtime's authoritative on-disk copy) wired pre-container-start on all three providers. Restored-blob mode is per-provider (0600 root→1000 on AWS/remote encrypted storage; 0644 on local, matching its other container-read data-dir files). Two failure modes: A (credential lost → restore fixes silently) vs B (refresh-token expiry/revocation → still needs a browser re-auth, by design). Reviews caught + fixed a non-root ownership bug (blob unreadable) and a **critical AWS bug** (SSM `RunCommand` returns `(result,nil)` for Failed status → existence probe always true → AWS restore was a silent no-op; fixed to read stdout, unit-guarded). Integration tests (`-tags integration`) for local + remote restart-restore; AWS unit/build-only per `project_no_aws_ci_integration`. **Live-verified on AWS `team-b`**: capture → delete on-disk blob → `conga refresh` restored it byte-identical (sha256 match), 0600 node-owned; cold-only left the present github blob untouched; `conga doctor` exit 0. Deferred (optional): re-capture on refresh (§4.4 freshness), `ListSecrets` blob-display polish, `team-b` GitHub PAT→OAuth migration. See `specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/`.
