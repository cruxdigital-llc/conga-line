...
- **Milestone**: the operator's local gateway replaced by AWS deployment
...
- [x] Phase 8 — Verification: full suite green (21 packages), vet
  ...
  user-a deployed and running on Opus 4.7; deployed openclaw.json
...
- [x] **Bonus mid-session**: bumped runtime default Opus 4-6 → 4-7
  (commit `3505f20`). user-a now runs `claude-opus-4-7
...
- [ ] **Pending**: live chat smoke (<operator> DM → user-a, observe
...
- **Approach C (recommended, live-validated on `user-a`/`2026.5.26`)**: layered config via OpenClaw's
...
- **Shipped (2026-06-09)**: implemented across all three providers + boot tftpl + provision scripts;
  ...
  AWS fleet (2026-06-10)**: refreshed all 5 agents (user-a, team-a, user-c, user-b,
  team-c) through the real `regenerateAgentConfigOnInstance` code path — each got
...
> feature **#33 (Managed-Host Migration Hardening)**. 3 of 6 agents migrated (user-a/user-c/team-b
...
- **Four defects (diagnosed live)**: (1) **HIGH** egress-proxy IP collision — proxy runs `--restart always`
  ...
  (`team-b` needed a fleet-bouncing `docker` daemon restart). (3) **MED** `refresh-all` shares one
...
### Remote-MCP OAuth Credential Lifecycle — Phase 1 implemented + live-verified; Phase 2 pending
*Lead: Architect + QA + PM. Branch: `plan/fleet-mcp-oauth-provisioning`*
- **Origin**: Live `team-a` "no Linear access" incident (2026-07-22). Ruled out the VPN/egress
  hypothesis; real cause = the per-container OAuth credential (`/home/node/.openclaw/mcp-oauth/<server>-<hash>.json`,
  holding the refresh token) had gone missing. This state is invisible to the declarative fleet and
  vanishes on any container/host lifecycle event; recovery today is a manual, undocumented browser flow.
  Fixed live via `openclaw mcp login linear` — does not scale as teams self-serve MCP servers.
- **Chosen direction (operator)**: two-phase. **P1** detect (`conga doctor` fleet scan for
  `bundle-mcp … requires OAuth`) + one-command re-auth (`conga mcp login <agent> <server>`, all providers)
  + docs. **P2** persist the OAuth blob to the per-agent secrets store on login, restore into the
  container dataDir before start on provision/refresh (survives host replacement). Runtime-boundary aware
  (OpenClaw now, Hermes later).
- [x] Requirements: `specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/requirements.md`
- [x] Plan (grounded in code anchors): `specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/plan.md`
- [x] Spec: `.../spec.md` — open Qs resolved **empirically** on the live fleet: Q1 filename hash is
  deterministic (store/restore by exact name); Q2 → two-failure-mode model (A: cred lost → restore fixes;
  B: token expired/revoked → only re-auth fixes); Q4 no `mcp list --status` in 2026.6.5 → log-scan
  detection. Data-safety tension (blob in data dir) resolved via cold-only non-destructive restore.
- [x] Persona review (Architect/QA/PM) — all APPROVE (2 amendments folded: capture-cadence residual
  risk §4.4; criterion 7 scoped to Mode A).
- [x] Standards gate (pre-implementation) — **PASS**, no `must` violation. One tracked impl gate:
  audit `GenerateEnvFile`/`SecretNameToEnvVar` callers so the `mcp-oauth/` secret prefix never leaks
  into the env file.
- [x] **Phase 1 implemented + live-verified** (`internal/` only, **no provider release**): `conga mcp
  login [server] --agent <name>` (leg-1 URL / leg-2 `--code` / interactive / idempotent) + `conga
  doctor` (fleet or `--agent` OAuth log-scan, text non-zero-exit + JSON), both with MCP-tool +
  `json-schema` parity. Shared pure logic in new `internal/mcpoauth` pkg (unit-tested). Verified live on
  AWS. Known limitation: `doctor` log-scan can false-positive right after a fix until the stale error
  ages out of the window (no health API in 2026.6.5) — mitigated by reporting the last-error timestamp.
- [ ] **Phase 2 pending** (`/glados:implement-feature` again): S1 `Runtime.OAuthStateDir()` + secret-prefix
  skip, S4 `CaptureMCPOAuth`, S5 cold-only `RestoreMCPOAuth`, S7 data-safety tests. Touches `pkg/` →
  needs a `terraform-provider-conga` release.
- **TODO (post-feature, decided 2026-07-22)**: retire `team-b`'s GitHub **PAT** and switch to
  **OAuth**. Verified the server uses a static `github_pat_…` embedded in `custom.json` (no `auth: oauth`;
  the OAuth blob is a stale orphan) — contra security.md §5 + inaccurate tfvars comment. Sequenced after
  this feature (its lifecycle machinery will manage the new OAuth cred). Steps in `spec.md` appendix.
  **Rotate the exposed PAT independently/ASAP** (surfaced in tool output) — does not depend on the feature.
...
- **Remote-MCP OAuth creds are unmanaged per-container state (planning underway)**: OAuth-authenticated MCP servers (Linear on `team-a` + `team-b`; GitHub on `team-b`) store their credential at `/home/node/.openclaw/mcp-oauth/<server>-<hash>.json` inside the container — invisible to tfvars/secrets store, wiped on any reprovision/host-replacement, silently failing with `bundle-mcp … requires OAuth`. Recovery is a manual browser flow. Fix being planned in `specs/2026-07-22_feature_mcp-oauth-credential-lifecycle/` (detect + `conga mcp login` CLI, then persist/restore via the per-agent secrets store).
