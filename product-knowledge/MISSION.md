...
- **Provisioning logic in tested Go, not templated bash** — host lifecycle (systemd units, deterministic per-agent networks, fail-safe migration, egress wiring) is generated by a shared, unit-tested Go engine (`pkg/provider/managedhost/`) behind a thin `Transport` seam (SSM for AWS, SSH/SFTP for remote). Bash on the host only places files and runs short commands. This unifies the AWS + remote provisioning paths, eliminates untestable shell drift, and makes the fleet reboot-safe (deterministic IPs + pinned proxies → unattended whole-fleet recovery).
