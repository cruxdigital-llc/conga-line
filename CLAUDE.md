...
- **AWS Account**: 167595588574, region us-east-2
...
- **Terraform state**: S3 bucket `openclaw-terraform-state-167595588574` + DynamoDB `openclaw-terraform-locks`
...
- Secrets are in AWS Secrets Manager under `openclaw/shared/*` and `openclaw/aaron/*`
...
- Slack channel for Aaron: `C0ALL272SV8`
...
- Connect to instance: `aws ssm start-session --target <instance-id> --region us-east-2 --profile 167595588574_AdministratorAccess`
- Bootstrap log: `cat /var/log/openclaw-bootstrap.log`
- Service status: `systemctl status openclaw-aaron`
- Container logs: `docker logs openclaw-aaron --tail 50`
- Journal: `journalctl -u openclaw-aaron --no-pager -n 50`
